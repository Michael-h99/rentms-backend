...
const getLandlordPlazas = asyncHandler(async (req, res) => {
  ...
  const [rows] = await db.query(
    "SELECT p.id, p.name, p.location, p.total_units, p.created_at, COUNT(CASE WHEN t.status = 'active' THEN 1 END) AS occupied_units, p.total_units - COUNT(CASE WHEN t.status = 'active' THEN 1 END) AS vacant_units FROM plazas p LEFT JOIN tenancies t ON t.plaza_id = p.id WHERE p.landlord_id = ? AND p.deleted_at IS NULL GROUP BY p.id ORDER BY p.created_at DESC LIMIT ? OFFSET ?",
...
const getPlazaById = asyncHandler(async (req, res) => {
  ...
  const [[stats]] = await db.query(
    "SELECT COUNT(CASE WHEN t.status = 'active' THEN 1 END) AS active_tenants, COUNT(CASE WHEN t.status = 'expired' THEN 1 END) AS expired_tenants, COALESCE(SUM(CASE WHEN py.status = 'paid' AND YEAR(py.payment_date) = YEAR(CURDATE()) AND MONTH(py.payment_date) = MONTH(CURDATE()) THEN py.amount END), 0) AS revenue_this_month, COUNT(CASE WHEN mr.status IN ('pending','in_progress') THEN 1 END) AS open_maintenance FROM tenancies t LEFT JOIN payments py ON py.tenancy_id = t.id LEFT JOIN maintenance_requests mr ON mr.plaza_id = t.plaza_id WHERE t.plaza_id = ?",
    [Number(plazaId)],
  );
...
const getPlazaTenants = asyncHandler(async (req, res) => {
  ...
  const [rows] = await db.query(
    `SELECT t.id, t.tenant_id, t.unit_number, t.rent_amount, t.lease_start, t.lease_end, t.status, t.created_at, u.full_name, u.email, u.phone, u.avatar_url FROM tenancies t JOIN users u ON u.id = t.tenant_id WHERE ${WHERE} ORDER BY t.created_at DESC LIMIT ? OFFSET ?`,
...
const removeTenant = asyncHandler(async (req, res) => {
  ...
  const [[tenancy]] = await db.execute(
    "SELECT t.id, t.tenant_id, t.status, u.full_name AS tenant_name, p.name AS plaza_name FROM tenancies t JOIN plazas p ON p.id = t.plaza_id JOIN users u ON u.id = t.tenant_id WHERE t.id = ? AND p.landlord_id = ?",
...
const getRentPayments = asyncHandler(async (req, res) => {
  ...
  const [rows] = await db.query(
    `SELECT pay.id, pay.amount, pay.currency, pay.payment_method, pay.status, pay.reference, pay.payment_date, pay.verified_at, t.unit_number, u.full_name AS tenant_name, u.email AS tenant_email, p.id AS plaza_id, p.name AS plaza_name FROM payments pay JOIN tenancies t ON t.id = pay.tenancy_id JOIN plazas p ON p.id = t.plaza_id JOIN users u ON u.id = t.tenant_id WHERE ${WHERE} ORDER BY pay.payment_date DESC LIMIT ? OFFSET ?`,
    [...params, Number(limit), Number(offset)],
  );
  return res.json({
    success: true,
...
const getMaintenanceRequests = asyncHandler(async (req, res) => {
  ...
  const [rows] = await db.query(
    `SELECT mr.id, mr.title, mr.description, mr.priority, mr.status, mr.attachment_url, mr.resolved_at, mr.created_at, mr.updated_at, p.id AS plaza_id, p.name AS plaza_name, u.full_name AS tenant_name, u.email AS tenant_email FROM maintenance_requests mr JOIN plazas p ON p.id = mr.plaza_id JOIN users u ON u.id = mr.tenant_id WHERE ${WHERE} ORDER BY CASE mr.priority WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END, mr.created_at DESC LIMIT ? OFFSET ?`,
    [...params, Number(limit), Number(offset)],
  );
  return res.json({
    success: true,
...
const updateMaintenanceStatus = asyncHandler(async (req, res) => {
  ...
  const [[row]] = await db.execute(
    "SELECT mr.id, mr.status AS old_status, mr.tenant_id, mr.title, p.landlord_id FROM maintenance_requests mr JOIN plazas p ON p.id = mr.plaza_id WHERE mr.id = ?",
    [Number(requestId)],
  );
  ...
  await db.execute(
    ...
    [Number(requestId), landlordId, row.old_status, status, note || null],
  );
  await NotificationService.create({
  ...
  await logActivity(
    ...
    `Updated request ${requestId}: "${row.old_status}" to "${status}"`,
    { ip: req.ip },
  );
  return res.json({
    success: true,
...
const createPlazaGroup = asyncHandler(async (req, res) => {
  ...
  const groupInviteCode = generateInviteCode();
  const [result] = await db.execute(
...
const getLandlordGroups = asyncHandler(async (req, res) => {
  ...
  const [rows] = await db.execute(
    `SELECT pg.id, pg.name, pg.invite_code, pg.created_at, p.id AS plaza_id, p.name AS plaza_name, COUNT(DISTINCT gm.sender_id) AS message_count FROM plaza_groups pg JOIN plazas p ON p.id = pg.plaza_id LEFT JOIN group_messages gm ON gm.group_id = pg.id WHERE ${conditions.join(" AND ")} GROUP BY pg.id ORDER BY pg.created_at DESC`,
    params,
  );
...
const getGroupMessages = asyncHandler(async (req, res) => {
  ...
  const [messages] = await db.execute(
    "SELECT gm.id, gm.sender_id, gm.content, gm.file_url, gm.file_type, gm.created_at, u.full_name AS sender_name, u.avatar_url AS sender_avatar, u.role AS sender_role FROM group_messages gm JOIN users u ON u.id = gm.sender_id WHERE gm.group_id = ? ORDER BY gm.created_at ASC LIMIT ? OFFSET ?",
...
const sendGroupMessageLandlord = asyncHandler(async (req, res) => {
  ...
  const [result] = await db.execute(
    ...
    [Number(groupId), landlordId, content?.trim() || null, file_url, file_type],
  );
